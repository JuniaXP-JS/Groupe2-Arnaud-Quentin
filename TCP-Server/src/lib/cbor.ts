...
export function encode(value: any): string | Uint8Array {
    ...
    function prepareWrite(length: number): PrepareWriteResult {
    ...
    function writeUint64(value: number): void {
        let low: number = value % POW_2_32;
        let high: number = (value - low) / POW_2_32;
    ...
    interface WriteTypeAndLength {
        (type: number, length: number): void;
    }

    const writeTypeAndLength: WriteTypeAndLength = function (type, length) {
        if (length < 24) {
...
export function decode(data: Uint8Array, tagger?: any, simpleValue?: any): any {
    let dataView = new DataView(data.buffer);
    let offset = 0;
    if (typeof tagger !== "function") tagger = (value: any): any => value;
    ...
    function readArrayBuffer(length: number): Uint8Array {
        return commitRead(length, new Uint8Array(data.buffer, offset, length));
    }
    ...
    interface ReadIndefiniteStringLength {
        (majorType: number): number;
    }

    const readIndefiniteStringLength: ReadIndefiniteStringLength = function (majorType) {
        let initialByte: number = readUint8();
        if (initialByte === 0xff) return -1;
        let length: number = readLength(initialByte & 0x1f);
    ...
    function decodeItem(): any {
        ...
        switch (majorType) {
            ...
            case 4:
                ...
                return retArray;
            case 5:
                let retObject: Record<string, any> = {};
